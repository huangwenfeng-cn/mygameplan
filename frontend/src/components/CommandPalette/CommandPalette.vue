...
function getLocalizedSearchGroupTitle(title: string) {
  if (title === 'Discussions') return '讨论'
  if (title === 'Tasks') return '任务'
  if (title === 'Pages') return '页面'
  return title
}
...
const shortcuts = computed((): CommandPaletteGroup[] => [
  ...
  },
  {
    title: (() => {
      ...
      return space.value ? `在 ${space.value.title} 中新建` : '新建'
    })(),
    items: [
      {
        title: '新建讨论',
...
function generateSearchResults() {
  let groups = [{ title: '空间', component: markRaw(ItemProject) }, { title: '成员' }]
