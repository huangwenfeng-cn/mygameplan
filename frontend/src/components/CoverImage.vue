<template>
  <div class="min-h-[3rem] overflow-hidden bg-surface-gray-2">
    <div class="group relative h-[130px] w-full" v-if="validatedImageUrl">
      ...
      >
        <div class="text-center">
          <div class="rounded-md py-1 text-lg text-ink-white">上下拖动以调整图片位置</div>
          <Button
            class="mt-2"
            @click="
              () => {
          ...
          <Button class="ml-2 mt-2" @click="reposition = false">取消</Button>
        </div>
      </div>
      <div
      ...
      >
        ...
        >
          <template v-slot="{ togglePopover }">
            <Button variant="outline" @click="togglePopover()"> 更换图片 </Button>
          </template>
        </UnsplashImageBrowser>
        <Button
          v-if="editable"
    ...
    >
      ...
      >
        <template v-slot="{ togglePopover }">
          <Button variant="outline" @click="togglePopover()"> 点击设置封面图 </Button>
        </template>
      </UnsplashImageBrowser>
    </div>
  </div>
