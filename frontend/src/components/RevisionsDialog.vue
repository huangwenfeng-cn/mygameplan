        ...
        <div class="">
          <div class="max-h-[60vh] space-y-1 -m-1 p-1 overflow-y-auto" role="listbox">
            <button
              ...
              :aria-label="`修订于 ${dayjsLocal(revision.creation).format('LLL')}`"
              :class="
...
const title = computed(() => {
  if (revisions.data) {
    if (revisions.data.length === 0) return '无修订记录'
    if (revisions.data.length === 1) return '1 条修订'
    return `${revisions.data.length} 条修订`
  }
  return '加载中...'
})
