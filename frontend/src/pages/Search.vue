        ...
        <div class="mt-2 text-sm flex items-center justify-between min-h-6">
          <div>
            ...
            <template v-else-if="newSearch && query.length > 3">
              <p class="text-ink-gray-6">按回车开始搜索</p>
            </template>
            <template v-else-if="search.loading">
              <p class="text-ink-gray-6">搜索中...</p>
            </template>
            <template v-else-if="searchResponse?.summary">
              <div class="space-y-1">
                ...
                <p v-if="searchResponse.summary.corrected_query" class="text-ink-gray-6">
                  <span class="text-ink-gray-5">已修正为：</span>
          ...
          >
            <span class="text-ink-gray-6">结果有帮助吗？</span>
          ...
          <div v-else-if="feedbackGiven" class="text-ink-gray-6">感谢你的反馈！</div>
        </div>
...
const doctypesFilterOptions = computed(() => {
  ...
  const doctypeMapping = [
    { value: 'GP Discussion', label: '讨论' },
    { value: 'GP Task', label: '任务' },
    { value: 'GP Page', label: '页面' },
    { value: 'GP Comment', label: '评论' },
  ]
