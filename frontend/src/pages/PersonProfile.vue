    ...
    <div class="-mt-4 body-container">
      <div class="flex items-center">
        <div class="-mx-1 inline-flex translate-y-0">
          ...
          >
            <span v-if="$isSessionUser(profile.user)">上传头像</span>
          </button>
        </div>
        ...
        <div v-if="$isSessionUser(profile.user)">
          ...
          <Button label="编辑资料" @click="editDialog.show = true" class="sm:hidden">
        ...
        <TabButtons
          ...
          :buttons="
            [
              ...
              { label: '帖子' },
              { label: '回复' },
              $isSessionUser(profile.user) ? { label: '书签' } : null,
            ].filter(Boolean)
          "
          v-model="activeTab"
        />
    ...
    activeTab: {
      ...
      set(value) {
        let route = {
          ...
          书签: { name: 'PersonProfileBookmarks' },
        }[value]
        if (route) {
