<template>
  <PageHeader>
    <Breadcrumbs class="h-7" :items="[{ label: '草稿', route: { name: 'Drafts' } }]" />
    <div class="flex items-center gap-2">
      ...
      <template v-else>
        <Button variant="ghost" @click="cancelBulkDelete">取消</Button>
...
function deleteDrafts() {
  deleteDraftsCall
    ...
    .then(() => {
      ...
      if (deletedCount > 0) {
        toast.success(deletedCount === 1 ? '已删除草稿' : `已删除 ${deletedCount} 条草稿`)
      }
